...
For example, you can start the `memgraph` Docker image with `docker run
memgraph/memgraph --bolt-port=7687 --log-level=TRACE`, but you should start
`memgraph-platform` Docker image with `docker run -p 7687:7687 -p 7444:7444 -p
3000:3000 -e MEMGRAPH="--bolt-port=7687 --log-level=TRACE"
...
| --auth-module-create-missing-user| Controls whether users should be implicitly created on first login or they should be explicitly created manually.                                |
| --auth-module-create-missing-role| Controls whether roles should be implicitly created on first appearance or they should be explicitly created manually.                           |
...
| Flag                                     | Description                                                                                                 | Type       |
|------------------------------------------|-------------------------------------------------------------------------------------------------------------|------------|
| --raft-server-id                | List of Kafka brokers as a comma separated list of broker `host` or `host:port`.                            | `[string]` |
| --pulsar-service-url                     | The service URL that will allow Memgraph to locate the Pulsar cluster.                                      | `[string]` |
| --stream-transaction-conflict-retries=30 | Number of times to retry a conflicting transaction of a stream.                                             | `[uint32]` |
| --stream-transaction-retry-interval=500  | The interval to wait (measured in milliseconds) before retrying to execute again a conflicting transaction. | `[uint32]` |
...
be changed in the configuration file or by passing configuration flags.
...
![](/pages/configuration/configuration-settings/memgraph-lab-init-file.png)
...
![](/pages/configuration/configuration-settings/memgraph-lab-init-data-file.png)
